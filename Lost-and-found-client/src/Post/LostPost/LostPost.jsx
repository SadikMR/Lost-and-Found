...
const LostPost = () => {
    ...
    const handleLostpost = (e) => {
        ...
        const lostInfo = { category, productName, color, brand, description, possibleLocation, possibleDate, image };
        console.log(category, productName, color, brand, description, possibleLocation, possibleDate, image);

        fetch('http://localhost:5000/lostpost', {
            method: 'POST',
            headers: {
    ...
    return (
        <div className="flex items-center justify-center min-h-screen mb-5">
            <div className=" max-w-3xl bg-[#EADBC8] rounded-lg shadow-lg p-8">
                {/* Heading */}
                <h2 className="text-2xl font-bold text-center text-gray-800">Lost Form : {lostpostInfo.length}</h2>
                <p className="text-center text-gray-600 mt-2">Fill out the form to submit a new post</p>

                {/* Form */}
                <form onSubmit={handleLostpost} className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                    {/* Category */}
                    <div>
